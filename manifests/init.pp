class phalcon::install {
    ...
    file { '/etc/php5/conf.d/phalcon.ini':
        content => 'extension=phalcon.so',
        require => Exec['install-phalcon-sh']
    }

    file { '/etc/php5/cli/phalcon.ini':
        ensure => link,
        target => 'etc/php5/conf.d/phalcon.ini'
    }

    file { '/etc/php5/fpm/phalcon.ini':
        ensure => link,
        target => 'etc/php5/conf.d/phalcon.ini'
    }

    notify { 'Phalcon ini extension has been written sucessfully' : 
		require => File['/etc/php5/conf.d/phalcon.ini']
	}
